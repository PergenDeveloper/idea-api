...
class AccountRegister(graphene.Mutation):
    ...
    class Arguments:
        input = AccountRegisterInput(
            required=True, description="Fields required to create an user."
        )

    @classmethod
    def clean_input(cls, info, data):
        """Return the input data after all checks and cleanups."""
        ...
        if not valid_password(data["password"]):
            raise GraphQLError("Invalid password. Must be at least 8 characters.")
...
class PasswordChange(graphene.Mutation):
    ...
    def clean_input(cls, user, data):
        ...
        if not valid_password(data["new_password"]):
            raise GraphQLError("Invalid new password. Must be at least 8 characters.")
...
class PasswordResetRequest(graphene.Mutation):
    ...
    def mutate(cls, _, info, **kwargs):
        ...
        onetimetoken = models.OneTimeToken.objects.filter(user=user, valid=True).first()
        if not onetimetoken:
...
class PasswordResetConfirm(graphene.Mutation):
    ...
    class Arguments:
        ...
        password = graphene.String(required=True, description="New password of user.")
    ...
    def mutate(cls, _, info, **kwargs):
        ...
        onetimetoken = (
            models.OneTimeToken.objects.select_related("user")
            .filter(
                token=token,
                valid=True,
            )
