...
class PublicationCreateInput(graphene.InputObjectType):
    ...
    visibility = graphene.Field(PublicationVisibilityEnum, required=True)


class PublicationUpdateInput(graphene.InputObjectType):
    text = graphene.String(required=False, description="Text of publication.")
    visibility = graphene.Field(PublicationVisibilityEnum, required=False)
...
class PublicationCreate(graphene.Mutation):
    ...
    class Arguments:
        input = PublicationCreateInput(
            required=True, description="Fields required to create a publication."
        )
    ...
    def mutate(cls, _, info, **kwargs):
        ...
        publication = models.Publication(user=user, **data)
        publication.save()
...
class PublicationUpdate(graphene.Mutation):
    ...
    class Arguments:
        id = graphene.UUID(required=True, description="ID of publication.")
